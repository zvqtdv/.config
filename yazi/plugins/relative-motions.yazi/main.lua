...
local MOTIONS_AND_OP_KEYS = {
	...
	{ on = "W" }, { on = "<" }, { on = ">" }, { on = "~" },
	-- movement
	-- { on = "g" }, { on = "n" }, { on = "e" }, { on = "h" }, { on = "l" }, { on = "<Down>" }, { on = "<Up>" }, { on = "<Left>" }, { on = "<Right>" }
	{ on = "g" }, { on = "n" }, { on = "e" }, { on = "h" }, { on = "o" }, { on = "<Down>" }, { on = "<Up>" }, { on = "<Left>" }, { on = "<Right>" }
}
...
local MOTION_KEYS = {
	...
	-- movement
	-- { on = "g" }, { on = "n" }, { on = "e" }, { on = "h" }, { on = "l" }, { on = "<Down>" }, { on = "<Up>" }, { on = "<Left>" }, { on = "<Right>" }
	{ on = "g" }, { on = "n" }, { on = "e" }, { on = "y" }, { on = "o" }, { on = "<Down>" }, { on = "<Up>" }, { on = "<Left>" }, { on = "<Right>" }
}

-- stylua: ignore
local DIRECTION_KEYS = {
	-- { on = "n" }, { on = "e" }, { on = "<Down>" }, { on = "<Up>" },
	{ on = "n" }, { on = "e" }, { on = "<Down>" }, { on = "<Up>" },
	-- tab movement
	{ on = "t" }
}
...
local function get_cmd(first_char, keys)
	...
	-- command direction
	local direction
	-- if last_key == "g" or last_key == "v" or last_key == "d" or last_key == "h" or last_key == "x" then
	if last_key == "g" or last_key == "v" or last_key == "d" or last_key == "h" or last_key == "x" then
